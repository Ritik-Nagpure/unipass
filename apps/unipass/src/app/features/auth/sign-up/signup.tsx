...
const SignUpForm: React.FC<SignUpFormProps> = ({ onSwitchToLogin }) => {
  ...
  return (
    <div className="w-full max-w-md mx-auto flex flex-col justify-center h-full">
      <h2 className="text-4xl font-semibold text-white mb-2">Create an account</h2>
      <p className="text-gray-400 text-sm mb-8">
        Already have an account?{' '}
        <button onClick={onSwitchToLogin} className="text-white underline hover:text-gray-300 transition-colors">
          Log in
        </button>
      </p>
      ...
      <form className="space-y-4">
        ...
        <div className="flex gap-4">
          <input
            ...
            className="w-1/2 px-4 py-3 bg-[#2D2D3A] border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-[#7C5CFC] transition-colors"
          />
          <input
            type="text"
        ...
        <div className="flex items-center gap-3 pt-2">
          <div className="relative flex items-center">
            <input
              type="checkbox"
              id="terms"
              className="peer h-5 w-5 cursor-pointer appearance-none rounded border border-gray-500 checked:border-[#7C5CFC] checked:bg-[#7C5CFC] transition-all"
            />
            <svg
      ...
      <div className="flex items-center my-6">
        ...
        <span className="px-4 text-xs text-gray-400">Or register with</span>
        <div className="flex-1 h-px bg-gray-600" />
      </div>
